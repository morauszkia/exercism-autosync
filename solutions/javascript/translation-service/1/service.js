...
export class TranslationService {
  ...
  request(text) {
    ...
    return makeRequest().catch(makeRequest).catch(makeRequest);
  }
  ...
  premium(text, minimumQuality) {
    return this.api.fetch(text)
      // If there is no translation stored, catches forwarded error and requests translation
      // If request successful, fetches translation
      .catch(() => this.request(text).then(() => this.api.fetch(text)))
      // If fetch successful, checks quality
      .then(result => {
        // Throws error if quality is below threshold
