...
pub fn count_place_treasures(
  ...
  treasures
  |> list.filter(fn(treasure) { pair.second(treasure) == place_location_to_treasure_location(pair.second(place)) })
  |> list.length
}
...
pub fn special_case_swap_possible(
  ...
  case 
    ...
    pair.first(place) {
      "Brass Spyglass", _, "Abandoned Lighthouse" |
      "Amethyst Octopus", "Crystal Crab", "Stormy Breakwater" |
      "Amethyst Octopus", "Glass Starfish", "Stormy Breakwater" |
